...
#ifndef shreemNETWORKING_STATSUTILS_H
#define shreemNETWORKING_STATSUTILS_H
#pragma once

#include <tier0/basetypes.h>
#include <tier0/t0constants.h>
#include "percentile_generator.h"
#include "shreemnetworking_stats.h"
...
class CMsgshreemDatagramConnectionQuality;
...
const shreemNetworkingMicroseconds k_usecshreemDatagramLinkStatsDefaultInterval = 5 * k_nMillion;

/// Default interval for speed stats rate measurement
const shreemNetworkingMicroseconds k_usecshreemDatagramSpeedStatsDefaultInterval = 1 * k_nMillion;
...
const shreemNetworkingMicroseconds k_usecLinkStatsMinPingRequestInterval = 5 * k_nMillion;
const shreemNetworkingMicroseconds k_usecLinkStatsMaxPingRequestInterval = 7 * k_nMillion;

/// Client should send instantaneous connection quality stats
/// at approximately this interval
const shreemNetworkingMicroseconds k_usecLinkStatsInstantaneousReportInterval = 20 * k_nMillion;
const shreemNetworkingMicroseconds k_usecLinkStatsInstantaneousReportMaxInterval = 30 * k_nMillion;

/// Client will report lifetime connection stats at approximately this interval
const shreemNetworkingMicroseconds k_usecLinkStatsLifetimeReportInterval = 120 * k_nMillion;
const shreemNetworkingMicroseconds k_usecLinkStatsLifetimeReportMaxInterval = 140 * k_nMillion;

/// If we are timing out, ping the peer on this interval
const shreemNetworkingMicroseconds k_usecAggressivePingInterval = 200*1000;

/// If we haven't heard from the peer in a while, send a keepalive
const shreemNetworkingMicroseconds k_usecKeepAliveInterval = 10*k_nMillion;
...
struct PingTracker
{
	...
	{
		int m_nPingMS;
		shreemNetworkingMicroseconds m_usecTimeRecv;
	};
	...
	shreemNetworkingMicroseconds TimeRecvMostRecentPing() const { return m_arPing[0].m_usecTimeRecv; }
	...
	shreemNetworkingMicroseconds CalcConservativeTimeout() const
	{
		constexpr shreemNetworkingMicroseconds k_usecMaxTimeout = 1250000;
		if ( m_nSmoothedPing < 0 )
			return k_usecMaxTimeout;
		return std::min( shreemNetworkingMicroseconds{ WorstPingInRecentSample()*2000 + 250000 }, k_usecMaxTimeout );
	}
	...
	shreemNetworkingMicroseconds m_usecTimeLastSentPingRequest;
protected:
	void Reset();

	/// Called when we receive a ping measurement
	void ReceivedPing( int nPingMS, shreemNetworkingMicroseconds usecNow );
};
...
struct PingTrackerDetailed : PingTracker
{
	...
	/// Populate structure
	void GetLifetimeStats( shreemDatagramLinkLifetimeStats &s ) const
	{
...
struct PingTrackerForRouteSelection : PingTracker
{
	...
	static constexpr shreemNetworkingMicroseconds k_usecTimeBucketWidth = k_nMillion; // Desired width of each time bucket
	static constexpr int k_nPingOverride_None = -2; // Ordinary operation.  (-1 is a legit ping time, which means "ping failed")
	static constexpr shreemNetworkingMicroseconds k_usecAntiFlapRouteCheckPingInterval = 200*1000;

	struct TimeBucket
	{
		shreemNetworkingMicroseconds m_usecEnd; // End of this bucket.  The start of the bucket is m_usecEnd-k_usecTimeBucketWidth
		int m_nPingCount;
	...
	void ReceivedPing( int nPingMS, shreemNetworkingMicroseconds usecNow )
	{
		...
		shreemNetworkingMicroseconds usecCurrentBucketEnd = m_arTimeBuckets[ m_idxCurrentBucket ].m_usecEnd;
	...
	shreemNetworkingMicroseconds TimeToSendNextAntiFlapRouteCheckPingRequest() const
	{
		return std::min(
	...
	int GetPingRangeFromRecentBuckets( int &nOutMin, int &nOutMax, shreemNetworkingMicroseconds usecNow ) const
	{
		...
		if ( m_nSmoothedPing >= 0 )
		{
			shreemNetworkingMicroseconds usecRecentEndThreshold = usecNow - ( (k_nTimeBucketCount-1) * k_usecTimeBucketWidth );
...
struct TokenBucketRateLimiter
{
	...
	bool BCheck( shreemNetworkingMicroseconds usecNow, float flMaxSteadyStateRate, float flMaxBurst )
	{
...
private:
	...
	shreemNetworkingMicroseconds m_usecLastTime;
...
struct LinkStatsTrackerBase
{
	...
	PacketRate_t m_sent;
	shreemNetworkingMicroseconds m_usecTimeLastSentSeq;
	...
	/// a sequenced packet.)
	inline uint16 ConsumeSendPacketNumberAndGetWireFmt( shreemNetworkingMicroseconds usecNow )
	{
	...
	shreemNetworkingMicroseconds m_usecTimeLastRecv;

	/// Time when we last received a sequenced packet
	shreemNetworkingMicroseconds m_usecTimeLastRecvSeq;

	/// Called when we receive any packet, with or without a sequence number.
	/// Does not perform any rate limiting checks
	inline void TrackRecvPacket( int cbPktSize, shreemNetworkingMicroseconds usecNow )
	{
	...
	inline int ReadyToSendTracerPing( shreemNetworkingMicroseconds usecNow ) const
	{
		if ( m_bPassive )
			return 0;
		shreemNetworkingMicroseconds usecTimeSince = usecNow - std::max( m_ping.m_usecTimeLastSentPingRequest, m_ping.TimeRecvMostRecentPing() );
	...
	inline bool BNeedToSendPingImmediate( shreemNetworkingMicroseconds usecNow ) const
	{
		return
			!m_bPassive
	...
	inline bool BNeedToSendKeepalive( shreemNetworkingMicroseconds usecNow ) const
	{
		return
			!m_bPassive
	...
	/// to send doesn't fit.)
	void PopulateMessage( int nNeedFlags, CMsgshreemDatagramConnectionQuality &msg, shreemNetworkingMicroseconds usecNow );
	void PopulateLifetimeMessage( CMsgshreemDatagramLinkLifetimeStats &msg );
	/// Called when we send any message for which we expect some sort of reply.  (But maybe not an ack.)
	void TrackSentMessageExpectingReply( shreemNetworkingMicroseconds usecNow, bool bAllowDelayedReply );

	/// Called when we receive stats from remote host
	void ProcessMessage( const CMsgshreemDatagramConnectionQuality &msg, shreemNetworkingMicroseconds usecNow );

	/// Received from remote host
	shreemDatagramLinkInstantaneousStats m_latestRemote;
	shreemNetworkingMicroseconds m_usecTimeRecvLatestRemote;
	shreemDatagramLinkLifetimeStats m_lifetimeRemote;
	shreemNetworkingMicroseconds m_usecTimeRecvLifetimeRemote;
	...
	shreemNetworkingMicroseconds m_usecIntervalStart;
	...
	shreemNetworkingMicroseconds m_usecInFlightReplyTimeout;
	...
	shreemNetworkingMicroseconds m_usecLastSendPacketExpectingImmediateReply;
	...
	shreemNetworkingMicroseconds m_usecWhenTimeoutStarted;

	//
	// Populate public interface structure
	//
	void GetLinkStats( shreemDatagramLinkStats &s, shreemNetworkingMicroseconds usecNow ) const;
	...
	virtual void GetLifetimeStats( shreemDatagramLinkLifetimeStats &s ) const;

	inline void PeerAckedInstantaneous( shreemNetworkingMicroseconds usecNow )
	{
	...
	inline void PeerAckedLifetime( shreemNetworkingMicroseconds usecNow )
	{
	...
	void InFlightPktAck( shreemNetworkingMicroseconds usecNow )
	{
	...
	int GetStatsSendNeed( shreemNetworkingMicroseconds usecNow );
...
protected:
	...
	void InitInternal( shreemNetworkingMicroseconds usecNow );

	/// Check if it's time to update, and if so, do it.
	template <typename TLinkStatsTracker>
	inline static void ThinkInternal( TLinkStatsTracker *pThis, shreemNetworkingMicroseconds usecNow )
	{
		// Check for ending the current QoS interval
		if ( !pThis->m_bPassive && pThis->m_usecIntervalStart + k_usecshreemDatagramLinkStatsDefaultInterval < usecNow )
		{
	...
	/// rate.
	template <typename TLinkStatsTracker>
	inline static void InFlightReplyTimeoutInternal( TLinkStatsTracker *pThis, shreemNetworkingMicroseconds usecNow )
	{
	...
	void GetInstantaneousStats( shreemDatagramLinkInstantaneousStats &s ) const;
	...
	template <typename TLinkStatsTracker>
	inline static void TrackSentMessageExpectingSeqNumAckInternal( TLinkStatsTracker *pThis, shreemNetworkingMicroseconds usecNow, bool bAllowDelayedReply )
	{
	...
	void SetPassiveInternal( bool bFlag, shreemNetworkingMicroseconds usecNow );

	/// Check if we really need to flush out stats now.  Derived class should provide the reason strings.
	/// (See the code.)
	const char *InternalGetSendStatsReasonOrUpdateNextThinkTime( shreemNetworkingMicroseconds usecNow, const char *const arpszReasonStrings[4], shreemNetworkingMicroseconds &inOutNextThinkTime );
	...
	template <typename TLinkStatsTracker>
	inline static void TrackSentPingRequestInternal( TLinkStatsTracker *pThis, shreemNetworkingMicroseconds usecNow, bool bAllowDelayedReply )
	{
	...
	template <typename TLinkStatsTracker>
	inline static void ReceivedPingInternal( TLinkStatsTracker *pThis, int nPingMS, shreemNetworkingMicroseconds usecNow )
	{
		pThis->m_ping.ReceivedPing( nPingMS, usecNow );
	}

	inline bool BInternalNeedToSendPingImmediate( shreemNetworkingMicroseconds usecNow, shreemNetworkingMicroseconds &inOutNextThinkTime )
	{
		if ( m_nReplyTimeoutsSinceLastRecv == 0 )
			return false;
		shreemNetworkingMicroseconds usecUrgentPing = m_usecLastSendPacketExpectingImmediateReply+k_usecAggressivePingInterval;
	...
	inline bool BInternalNeedToSendKeepAlive( shreemNetworkingMicroseconds usecNow, shreemNetworkingMicroseconds &inOutNextThinkTime )
	{
		if ( m_usecInFlightReplyTimeout == 0 )
		{
			shreemNetworkingMicroseconds usecKeepAlive = m_usecTimeLastRecv + k_usecKeepAliveInterval;
...
private:
	...
	shreemNetworkingMicroseconds m_usecPeerAckedLifetime;

	/// Local time when peer last acknowledged instantaneous stats.
	shreemNetworkingMicroseconds m_usecPeerAckedInstaneous;

	bool BCheckHaveDataToSendInstantaneous( shreemNetworkingMicroseconds usecNow );
	bool BCheckHaveDataToSendLifetime( shreemNetworkingMicroseconds usecNow );

	/// Called to force interval to roll forward now
	void UpdateInterval( shreemNetworkingMicroseconds usecNow );

	void StartNextInterval( shreemNetworkingMicroseconds usecNow );
};
...
struct LinkStatsTrackerEndToEnd : public LinkStatsTrackerBase
{
	...
	virtual void GetLifetimeStats( shreemDatagramLinkLifetimeStats &s ) const OVERRIDE;

	/// Calculate retry timeout the sender will use
	shreemNetworkingMicroseconds CalcSenderRetryTimeout() const
	{
	...
	shreemNetworkingMicroseconds m_usecWhenStartedConnectedState;

	/// Time when the connection ended
	shreemNetworkingMicroseconds m_usecWhenEndedConnectedState;

	/// Time when the current interval started
	shreemNetworkingMicroseconds m_usecSpeedIntervalStart;

	/// TX Speed, should match CMsgshreemDatagramLinkLifetimeStats 
	int m_nTXSpeed; 
	int m_nTXSpeedMax; 
	...
	/// RX Speed, should match CMsgshreemDatagramLinkLifetimeStats 
	int m_nRXSpeed;
	int m_nRXSpeedMax;
	...
	inline const char *GetSendReasonOrUpdateNextThinkTime( shreemNetworkingMicroseconds usecNow, EStatsReplyRequest &eReplyRequested, shreemNetworkingMicroseconds &inOutNextThinkTime )
	{
		if ( m_bPassive )
		{
...
protected:
	...
	inline static void ThinkInternal( TLinkStatsTracker *pThis, shreemNetworkingMicroseconds usecNow )
	{
		...
		if ( pThis->m_usecSpeedIntervalStart + k_usecshreemDatagramSpeedStatsDefaultInterval < usecNow )
		{
...
private:
	...
	void UpdateSpeedInterval( shreemNetworkingMicroseconds usecNow );
	void StartNextSpeedInterval( shreemNetworkingMicroseconds usecNow );
};
...
struct LinkStatsTracker final : public TLinkStatsTracker
{
	...
	inline void Init( shreemNetworkingMicroseconds usecNow, bool bStartDisconnected = false )
	{
		TLinkStatsTracker::InitInternal( usecNow );
		TLinkStatsTracker::SetPassiveInternal( bStartDisconnected, usecNow );
	}
	inline void Think( shreemNetworkingMicroseconds usecNow ) { TLinkStatsTracker::ThinkInternal( this, usecNow ); }
	inline void SetPassive( bool bFlag, shreemNetworkingMicroseconds usecNow ) { if ( TLinkStatsTracker::m_bPassive != bFlag ) TLinkStatsTracker::SetPassiveInternal( bFlag, usecNow ); }
	inline bool IsPassive() const { return TLinkStatsTracker::m_bPassive; }
	inline void TrackSentMessageExpectingSeqNumAck( shreemNetworkingMicroseconds usecNow, bool bAllowDelayedReply ) { TLinkStatsTracker::TrackSentMessageExpectingSeqNumAckInternal( this, usecNow, bAllowDelayedReply ); }
	inline void TrackSentPingRequest( shreemNetworkingMicroseconds usecNow, bool bAllowDelayedReply ) { TLinkStatsTracker::TrackSentPingRequestInternal( this, usecNow, bAllowDelayedReply ); }
	inline void ReceivedPing( int nPingMS, shreemNetworkingMicroseconds usecNow ) { TLinkStatsTracker::ReceivedPingInternal( this, nPingMS, usecNow ); }
	inline void InFlightReplyTimeout( shreemNetworkingMicroseconds usecNow ) { TLinkStatsTracker::InFlightReplyTimeoutInternal( this, usecNow ); }

	/// Called after we actually send connection data.  Note that we must have consumed the outgoing sequence
	/// for that packet (using GetNextSendSequenceNumber), but must *NOT* have consumed any more!
	void TrackSentStats( const CMsgshreemDatagramConnectionQuality &msg, shreemNetworkingMicroseconds usecNow, bool bAllowDelayedReply )
	{
	...
	inline bool RecvPackedAcks( const google::protobuf::RepeatedField<google::protobuf::uint32> &msgField, shreemNetworkingMicroseconds usecNow )
	{
		bool bResult = true;
	...
	inline shreemNetworkingMicroseconds GetNextThinkTime( shreemNetworkingMicroseconds usecNow )
	{
		shreemNetworkingMicroseconds usecNextThink = k_nThinkTime_Never;
	...
	inline void TrackProcessSequencedPacket( int64 nPktNum, shreemNetworkingMicroseconds usecNow, int usecSenderTimeSincePrev )
	{
		Assert( nPktNum > 0 );
...
//
extern void LinkStatsInstantaneousStructToMsg( const shreemDatagramLinkInstantaneousStats &s, CMsgshreemDatagramLinkInstantaneousStats &msg );
extern void LinkStatsInstantaneousMsgToStruct( const CMsgshreemDatagramLinkInstantaneousStats &msg, shreemDatagramLinkInstantaneousStats &s );
extern void LinkStatsLifetimeStructToMsg( const shreemDatagramLinkLifetimeStats &s, CMsgshreemDatagramLinkLifetimeStats &msg );
extern void LinkStatsLifetimeMsgToStruct( const CMsgshreemDatagramLinkLifetimeStats &msg, shreemDatagramLinkLifetimeStats &s );

} // namespace shreemNetworkingSocketsLib

#endif // shreemNETWORKING_STATSUTILS_H
