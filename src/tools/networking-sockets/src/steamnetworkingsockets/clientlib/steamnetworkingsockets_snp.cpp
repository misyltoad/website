...
struct SNPAckSerializerHelper
{
	...
	static uint16 EncodeTimeSince( shreemNetworkingMicroseconds usecNow, shreemNetworkingMicroseconds usecWhenSentLast )
	{

		// Encode time since last
		shreemNetworkingMicroseconds usecElapsedSinceLast = usecNow - usecWhenSentLast;
...
// - clamp to minimum
inline shreemNetworkingMicroseconds GetUsecPingWithFallback( CshreemNetworkConnectionBase *pConnection )
{
...
void SSNPSenderState::RemoveAckedReliableMessageFromUnackedList()
{
	...
	{
		CshreemNetworkingMessage *pMsg = m_unackedReliableMessages.m_pFirst;
...
void SSNPSenderState::DebugCheckInFlightPacketMap() const
{
	...
	shreemNetworkingMicroseconds prevWhenSent = it->second.m_usecWhenSent;
...
//-----------------------------------------------------------------------------
void CshreemNetworkConnectionBase::SNP_InitializeConnection( shreemNetworkingMicroseconds usecNow )
{
	m_senderState.TokenBucket_Init( usecNow );

	shreemNetworkingMicroseconds usecPing = GetUsecPingWithFallback( this );
	...
	*/
	Assert( usecPing > 0 );
	int64 w_init = Clamp( 4380, 2 * k_cbshreemNetworkingSocketsMaxEncryptedPayloadSend, 4 * k_cbshreemNetworkingSocketsMaxEncryptedPayloadSend );
...
//-----------------------------------------------------------------------------
void CshreemNetworkConnectionBase::SNP_ShutdownConnection()
{
...
//-----------------------------------------------------------------------------
int64 CshreemNetworkConnectionBase::SNP_SendMessage( CshreemNetworkingMessage *pSendMessage, shreemNetworkingMicroseconds usecNow, bool *pbThinkImmediately )
{
	...
	if ( cbData > k_cbMaxUnreliableMsgSizeSend && !( pSendMessage->m_nFlags & k_nshreemNetworkingSend_Reliable )  )
	{
		SpewWarningRateLimited( usecNow, "Trying to send a very large (%d bytes) unreliable message.  Sending as reliable instead.\n", cbData );
		pSendMessage->m_nFlags |= k_nshreemNetworkingSend_Reliable;
	}

	if ( pSendMessage->m_nFlags & k_nshreemNetworkingSend_NoDelay )
	{
	...
	if ( pSendMessage->m_nFlags & k_nshreemNetworkingSend_Reliable )
	{
	...
	if ( pSendMessage->m_nFlags & k_nshreemNetworkingSend_NoNagle )
	...
	if ( GetState() == k_EshreemNetworkingConnectionState_Connected )
	{
		shreemNetworkingMicroseconds usecNextThink = SNP_GetNextThinkTime( usecNow );
		...
		else
		{
			...
			if ( pSendMessage->m_nFlags & k_nshreemNetworkingSend_UseCurrentThread )
			{
...
EResult CshreemNetworkConnectionBase::SNP_FlushMessage( shreemNetworkingMicroseconds usecNow )
{
...
bool CshreemNetworkConnectionBase::ProcessPlainTextDataChunk( int usecTimeSinceLast, RecvPacketContext_t &ctx )
{
	#define DECODE_ERROR( ... ) do { \
		ConnectionState_ProblemDetectedLocally( k_EshreemNetConnectionEnd_Misc_InternalError, __VA_ARGS__ ); \
	...
	const shreemNetworkingMicroseconds usecNow = ctx.m_usecNow;
	...
	while ( pDecode < pEnd )
	{
		...
		else if ( ( nFrameType & 0xf0 ) == 0x90 )
		{
			...
			// Parse out delay, and process the ping
			{
				...
				{
					shreemNetworkingMicroseconds usecDelay = shreemNetworkingMicroseconds( nPackedDelay ) << k_nAckDelayPrecisionShift;
					shreemNetworkingMicroseconds usecElapsed = usecNow - inFlightPkt->second.m_usecWhenSent;
			...
			if ( nBlocks > 0 )
			{
				...
				// packet!
				shreemNetworkingMicroseconds usecDelay = 250*1000 / nBlocks;
			...
			if ( bAckedReliableRange )
			{
				...
				if ( nLogLevelPacketDecode >= k_EshreemNetworkingSocketsDebugOutputType_Debug )
				{
...
void CshreemNetworkConnectionBase::SNP_SenderProcessPacketNack( int64 nPktNum, SNPInFlightPacket_t &pkt, const char *pszDebug )
{
...
shreemNetworkingMicroseconds CshreemNetworkConnectionBase::SNP_SenderCheckInFlightPackets( shreemNetworkingMicroseconds usecNow )
{
	...
	shreemNetworkingMicroseconds usecNextRetry = k_nThinkTime_Never;
	...
	shreemNetworkingMicroseconds usecRTO = m_statsEndToEnd.CalcSenderRetryTimeout();
	while ( m_senderState.m_itNextInFlightPacketToTimeout != m_senderState.m_mapInFlightPacketsByPktNum.end() )
	{
		...
		if ( !m_senderState.m_itNextInFlightPacketToTimeout->second.m_bNack )
		{
			...
			shreemNetworkingMicroseconds usecRetryPkt = m_senderState.m_itNextInFlightPacketToTimeout->second.m_usecWhenSent + usecRTO;
	...
	shreemNetworkingMicroseconds usecWhenExpiry = usecNow - usecRTO*2;
	for (;;)
	{
...
struct EncodedSegment
{
	...
	CshreemNetworkingMessage *m_pMsg;
	int m_cbSegSize;
	int m_nOffset;

	inline void SetupReliable( CshreemNetworkingMessage *pMsg, int64 nBegin, int64 nEnd, int64 nLastReliableStreamPosEnd )
	{
		Assert( nBegin < nEnd );
		//Assert( nBegin + k_cbshreemNetworkingSocketsMaxReliableMessageSegment >= nEnd ); // Max sure we don't exceed max segment size
	...
	inline void SetupUnreliable( CshreemNetworkingMessage *pMsg, int nOffset, int64 nLastMsgNum )
	{
...
bool CshreemNetworkConnectionBase::SNP_SendPacket( CConnectionTransport *pTransport, SendPacketContext_t &ctx )
{
	...
	shreemNetworkingMicroseconds usecNow = ctx.m_usecNow;
	...
	int cbMaxPlaintextPayload = std::max( 0, ctx.m_cbMaxEncryptedPayload-k_cbshreemNetwokingSocketsEncrytionTagSize );
	cbMaxPlaintextPayload = std::min( cbMaxPlaintextPayload, m_cbMaxPlaintextPayloadSend );

	uint8 payload[ k_cbshreemNetworkingSocketsMaxPlaintextPayloadSend ];
	...
	if ( m_senderState.m_listReadyRetryReliableRange.empty() )
	{
		...
		while ( cbBytesRemainingForSegments > 4 )
		{
			...
			CshreemNetworkingMessage *pSendMsg = m_senderState.m_messagesQueued.m_pFirst;
	...
	switch ( m_eNegotiatedCipher )
	{
		...
		case k_EshreemNetworkingSocketsCipher_AES_256_GCM:
		{
			...
			*(uint64 *)&m_cryptIVSend.m_buf += LittleQWord( m_statsEndToEnd.m_nNextSendSequenceNumber );
			...
			// Encrypt the chunk
			uint8 arEncryptedChunk[ k_cbshreemNetworkingSocketsMaxEncryptedPayloadSend + 64 ]; // Should not need pad
			...
			*(uint64 *)&m_cryptIVSend.m_buf -= LittleQWord( m_statsEndToEnd.m_nNextSendSequenceNumber );
			...
			Assert( (int)cbEncrypted <= k_cbshreemNetworkingSocketsMaxEncryptedPayloadSend ); // confirm that pad above was not necessary and we never exceed k_nMaxshreemDatagramTransportPayload, even after encrypting
...
void CshreemNetworkConnectionBase::SNP_SentNonDataPacket( CConnectionTransport *pTransport, shreemNetworkingMicroseconds usecNow )
{
...
void CshreemNetworkConnectionBase::SNP_GatherAckBlocks( SNPAckSerializerHelper &helper, shreemNetworkingMicroseconds usecNow )
{
	helper.m_nBlocks = 0;
	...
	shreemNetworkingMicroseconds usecSendAcksDueBefore = usecNow;
	shreemNetworkingMicroseconds usecTimeUntilNextPacket = shreemNetworkingMicroseconds( ( m_senderState.m_flTokenBucket - (float)m_cbMTUPacketSize ) / (float)m_senderState.m_n_x * -1e6 );
	...
	while ( n > 0 )
	{
		...
		int64 nAckEnd;
		shreemNetworkingMicroseconds usecWhenSentLast;
		if ( n == 0 )
		{
...
uint8 *CshreemNetworkConnectionBase::SNP_SerializeAckBlocks( const SNPAckSerializerHelper &helper, uint8 *pOut, const uint8 *pOutEnd, shreemNetworkingMicroseconds usecNow )
{
...
uint8 *CshreemNetworkConnectionBase::SNP_SerializeStopWaitingFrame( uint8 *pOut, const uint8 *pOutEnd, shreemNetworkingMicroseconds usecNow )
{
...
void CshreemNetworkConnectionBase::SNP_ReceiveUnreliableSegment( int64 nMsgNum, int nOffset, const void *pSegmentData, int cbSegmentSize, bool bLastSegmentInMessage, shreemNetworkingMicroseconds usecNow )
{
	...
	if ( nOffset == 0 && bLastSegmentInMessage )
	{
		...
		ReceivedMessage( pSegmentData, cbSegmentSize, nMsgNum, k_nshreemNetworkingSend_Unreliable, usecNow );
		return;
	}
	...
	CshreemNetworkingMessage *pMsg = CshreemNetworkingMessage::New( this, cbMessageSize, nMsgNum, k_nshreemNetworkingSend_Unreliable, usecNow );
	if ( !pMsg )
		return;
...
bool CshreemNetworkConnectionBase::SNP_ReceiveReliableSegment( int64 nPktNum, int64 nSegBegin, const uint8 *pSegmentData, int cbSegmentSize, shreemNetworkingMicroseconds usecNow )
{
	int nLogLevelPacketDecode = m_connectionConfig.m_LogLevel_PacketDecode.Get();
	...
	do
	{
		...
		if ( nHeaderByte & 0x80 )
		{
			ConnectionState_ProblemDetectedLocally( k_EshreemNetConnectionEnd_Misc_InternalError, "Invalid reliable message header byte 0x%02x", nHeaderByte );
			return false;
		}
		...
		if ( nHeaderByte & 0x40 )
		{
			...
			{
				ConnectionState_ProblemDetectedLocally( k_EshreemNetConnectionEnd_Misc_InternalError,
		...
		if ( !ReceivedMessage( pReliableDecode, cbMsgSize, nMsgNum, k_nshreemNetworkingSend_Reliable, usecNow ) )
...
void CshreemNetworkConnectionBase::SNP_RecordReceivedPktNum( int64 nPktNum, shreemNetworkingMicroseconds usecNow, bool bScheduleAck )
{
	...
	shreemNetworkingMicroseconds usecScheduleAck = bScheduleAck ? usecNow + k_usecMaxDataAckDelay : INT64_MAX;
	...
	else
	{
		...
		if ( itGap->second.m_nEnd-1 == nPktNum )
		{
			...
			if ( itGap->first == nPktNum )
			{
				...
				shreemNetworkingMicroseconds usecWhenAckPrior = itGap->second.m_usecWhenAckPrior;
		...
		if ( usecScheduleAck < itGap->second.m_usecWhenAckPrior )
		{
			...
			if ( usecScheduleAck <= m_receiverState.m_itPendingAck->second.m_usecWhenAckPrior )
			{
				...
				else
				{
					...
					shreemNetworkingMicroseconds usecOldSched = m_receiverState.m_itPendingAck->second.m_usecWhenAckPrior;
					while ( --m_receiverState.m_itPendingAck != itGap )
					{
...
int CshreemNetworkConnectionBase::SNP_ClampSendRate()
{
...
shreemNetworkingMicroseconds CshreemNetworkConnectionBase::SNP_ThinkSendState( shreemNetworkingMicroseconds usecNow )
{
	...
	shreemNetworkingMicroseconds usecNextAction = SNP_GetNextThinkTime( usecNow );
	Assert( usecNextAction > usecNow );
	return usecNextAction;
}

void CshreemNetworkConnectionBase::SNP_TokenBucket_Accumulate( shreemNetworkingMicroseconds usecNow )
{
...
void SSNPReceiverState::QueueFlushAllAcks( shreemNetworkingMicroseconds usecWhen )
{
...
{
	int64 nPrevEnd = 0;
	shreemNetworkingMicroseconds usecPrevAck = 0;
...
shreemNetworkingMicroseconds CshreemNetworkConnectionBase::SNP_TimeWhenWantToSendNextPacket() const
{
	...
	// Anything queued?
	shreemNetworkingMicroseconds usecNextSend;
	if ( m_senderState.m_messagesQueued.empty() )
	{
...
shreemNetworkingMicroseconds CshreemNetworkConnectionBase::SNP_GetNextThinkTime( shreemNetworkingMicroseconds usecNow )
{
	// We really shouldn't be trying to do this when not connected
	if ( !BStateIsConnectedForWirePurposes() )
	{
	...
	shreemNetworkingMicroseconds usecNextThink = m_receiverState.TimeWhenFlushAcks();
	...
	shreemNetworkingMicroseconds usecNextRetry = SNP_SenderCheckInFlightPackets( usecNow );

	// If we want to send packets, then we might need to wake up and take action
	shreemNetworkingMicroseconds usecTimeWantToSend = SNP_TimeWhenWantToSendNextPacket();
	...
	if ( usecTimeWantToSend < usecNextThink )
	{
		...
		shreemNetworkingMicroseconds usecNextSend = usecNow;
		shreemNetworkingMicroseconds usecQueueTime = m_senderState.CalcTimeUntilNextSend();
		if ( usecQueueTime > 0 )
		{
...
void CshreemNetworkConnectionBase::SNP_PopulateDetailedStats( shreemDatagramLinkStats &info )
{
...
void CshreemNetworkConnectionBase::SNP_PopulateQuickStats( shreemNetworkingQuickConnectionStatus &info, shreemNetworkingMicroseconds usecNow )
{
